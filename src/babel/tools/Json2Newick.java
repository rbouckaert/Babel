...
public class Json2Newick extends Runnable {
	...
	public void run() throws Exception {
		...
		//tree.close(out);
		out.println(toShortNewick(tree.getRoot()));
    ...
    public String toShortNewick(Node node) {
...
//		JSONObject node_attrs = jsonNode.getJSONObject("node_attrs");
		...
		node.metaDataString = "heights="+ height[1] + ",height_95%_HPD={" + height[0] + "," + height[2] +"}";
...
//		processMetaData(node);
	...
	/***
	 * Returns the height of a JSON node + confidence interval
	 * @param jsonNode - JSON encoding of a node
	 * @return the height of the node (either div or num_date) in order lower, mean, higher
	 * @throws JSONException
	 */
	private static double [] getHeight(JSONObject jsonNode) throws JSONException {
		...
		if (node_attrs.has("num_date")) {
			heights[1] = node_attrs.getJSONObject("num_date").getDouble("value");
			JSONArray confidence = node_attrs.getJSONObject("num_date").getJSONArray("confidence");
			heights[0] = confidence.getDouble(0);
			heights[2] = confidence.getDouble(1);
		}
		else if (jsonNode.has("div")) {
			heights[1] = jsonNode.getDouble("div");
			heights[0] = heights[1];
			heights[2] = heights[1];
		}
		else {
	...
	public static void main(String[] args) throws Exception {
		new Application(new Json2Newick(), "JSON to Newick", args);
	}
